...
def validate_args_fires(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        # swap gadm for admin
        valid_polyname_list = [
            "admin" if "gadm" in x else x for x in valid_polyname_list
        ]

        if polyname.lower() not in valid_polyname_list:
            raise Error(
                "For this batch service, polyname must one of: {}".format(
                    ", ".join(valid_polyname_list)
                )
            )
        ...
        if fire_type not in valid_fire_list:
            raise Error(
                "For this batch service, fire_type must one of {}".format(
                    ", ".join(valid_fire_list)
                )
            )
...
def validate_args_glad(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        params["gladConfirmOnly"] = (
            eval(gladConfirmOnly.title()) if gladConfirmOnly else False
        )
...
def get_iso_info(params, poly_iso_adm1_adm2_combos):
    ...
    if len(input_combo) > 1 and input_combo[0] == "global":
        raise Error(
            "if requesting globally summarized statistics, you cannot choose "
            "additional administrative units."
        )

    if input_combo[0] != "global" and input_combo not in iso_adm1_adm2_combos:
        raise Error(
            "That combination of admin units (ISO, adm1, adm2) does not exist. Please"
...
def validate_aggregate(params):
    ...
    if (agg_by and agg_admin) or (agg_by and agg_time):
        raise Error(
            "can't combine an aggregate_by with any other aggregation. "
    ...
    if input_agg_list and agg_values:
        ...
        if improper_agg_list:
            raise Error(
                "aggregate_by or aggregate_time or aggregate_admin must be "
        ...
        raise Error(
            "if aggregate_values is True, aggregate_by OR aggregate_admin OR aggregate_time parameters"
